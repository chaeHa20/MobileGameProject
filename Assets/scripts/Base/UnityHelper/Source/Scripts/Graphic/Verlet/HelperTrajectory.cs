...
namespace UnityHelper
{
    public class HelperTrajectory
    {
        ...
        private static List<Vector3> pathPointsList;// = new List<Vector3>();
		...
		static public Vector3 GetVelocityWithAngleAndTarget(Transform initialTransform, Vector3 destinationPos, float angle)
        {
            Vector3 initialPos = initialTransform.position;

            //Formula to calculate initial velocity
            //x = horizontal distance, y = altitude change
            //Vo = sqrt( (0.5 * gravity * x?* (tan쾎ngle + 1))  /  (x * tan(angle) - y) )
			...
			return initialVelocity * initialTransform.forward;
		}
        ...
        static public float GetAngle(float initialVelocity, Vector3 initialPosition, Vector3 targetPosition, bool useHighAngle)
        {
        ...
        static public float TimeToReachApex(Vector3 initialVelocity, float initialAngle)
        {
        ...
        /// <returns></returns>
        static public float TimeToReachTarget(Vector3 initialVelocity, float launchAngle, float startingYHeight, float targetYHeight)
        {
        ...
        static public float GetMaxRange(Vector3 initialVelocity, float initialAngle, float startYPos, float targetYPos)
        {
        ...
        /// <returns></returns>
        static public List<Vector3> GetTrajectoryPath(Vector3 initialVelocity, Vector3 startPos, int numOfPoints, float totalTime)
        {
            ...
            pathPointsList = new List<Vector3>();
